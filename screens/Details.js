...
const Details = ({ route, navigation }) => {
    ...
    return(
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <FlatList data={data.bids} renderItem={({ item }) => <DetailsBid bid={item} />} keyExtractor={(item) => item.id} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: SIZES.extraLarge*3 }}
                ListHeaderComponent={() => (
                    <React.Fragment>
                        <DetailsHeader data={data} navigations={navigation} />
                    </React.Fragment>
                )}
            />
        </SafeAreaView>
